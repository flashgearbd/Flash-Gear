...
function doGet(e){
  ...
  const products=values.slice(1).filter(r=>r.some(c=>String(c).trim()!=='')).map(row=>{
    ...
    return {id:o['Product ID']||'',name:o['Product Name']||'',category:o['Category']||'Gadgets',brand:o['Brand']||'',price:number(o['Price']),mrp:number(o['MRP']),stock:o['Stock']||'In Stock',warranty:o['Warranty']||'',image:o['Image URL']||'',description:o['Description']||'',featured:['yes','true','1'].includes(String(o['Featured']).toLowerCase())};
  }).filter(Boolean);
...
function saveProduct(form){
  ...
  const sheet=getSheet_();const headers=sheet.getRange(1,1,1,sheet.getLastColumn()).getDisplayValues()[0].map(h=>String(h).trim());
  const required=['Product ID','Product Name','Category','Brand','Price','MRP','Stock','Warranty','Image URL','Description','Featured','Active'];const missing=required.filter(h=>!headers.includes(h));if(missing.length)throw new Error('Missing columns: '+missing.join(', '));
  if(!String(form.name||'').trim())throw new Error('Product name is required.');if(!String(form.category||'').trim())throw new Error('Category is required.');
  let imageUrl=String(form.imageUrl||'').trim();if(form.imageData&&String(form.imageData).includes(','))imageUrl=saveImage_(form.imageData,form.imageName||'product.webp');
  const id=String(form.id||'').trim()||createProductId_(form.name);const values={'Product ID':id,'Product Name':String(form.name||'').trim(),'Category':String(form.category||'').trim(),'Brand':String(form.brand||'').trim(),'Price':cleanNumber_(form.price),'MRP':cleanNumber_(form.mrp),'Stock':String(form.stock||'In Stock').trim(),'Warranty':String(form.warranty||'').trim(),'Image URL':imageUrl,'Description':String(form.description||'').trim(),'Featured':form.featured?'Yes':'No','Active':form.active===false?'No':'Yes'};sheet.appendRow(headers.map(h=>values[h]??''));return {ok:true,id,imageUrl};
}
